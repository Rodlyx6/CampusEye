    ...
    <transition name="fade" mode="out-in">
      ...
      <section v-else-if="view==='user'" key="user" class="user relative">
        ...
        <transition name="fade-slow">
          <div v-if="showSplash" class="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-[#e9e5dc]/90 backdrop-blur-md">
            <div class="text-center animate-rise-up">
              ...
              <p class="text-slate-500 tracking-widest font-medium text-lg">CampusEye 校园导览系统</p>
        ...
        <header class="google-topbar">
          ...
          <div class="search-box">
            ...
            <div class="search-stats" v-if="keyword">{{filtered.length}} 结果</div>
          </div>
          <div class="actions">
                  ...
                  <div class="px-5">
                    ...
                    <div v-if="detailTab==='detail'">
                      <div class="detail-section">
                        ...
                        <div class="detail-text">{{active.detail||'暂无详细介绍'}}</div>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <section v-else key="admin" class="admin">
        <header class="google-topbar">
          ...
          <div class="search-box">
            <svg class="search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
            <input v-model.trim="ak" placeholder="筛选建筑..." />
          </div>
          <div class="actions">
...
import { currentUser, getBuildingDetail, getBuildingImages, getBuildingList, login, logout, register, replaceBuildingImages, updateBuilding, uploadFile, uploadFilesBatch } from './api/http'
...
const buildings=ref([]), keyword=ref(''), ak=ref('')
...
const filtered=computed(()=>!keyword.value?buildings.value:buildings.value.filter(b=>(b.name||'').includes(keyword.value)))
...
const load=async()=>{const r=await getBuildingList();buildings.value=Array.isArray(r.data?.data)?r.data.data:[]}
...
const goAdmin=async()=>{view.value='admin';if(form.id==null&&buildings.value.length)await pick(buildings.value[0].id)}

const openDetail=async(id)=>{const [d,im]=await Promise.all([getBuildingDetail(id),getBuildingImages(id)]);active.value=d.data?.data||null;viewerImages.value=(im.data?.data||[]).map(x=>x.imageUrl);idx.value=0;detailTab.value='overview'}
